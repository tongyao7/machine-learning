...
df.columns = ['sepal length', 'sepal width', 'pedal length', 'pedal width', 'label']
...
class KdTree:
    ...
    def create(self, dataSet, depth=0):
        if len(dataSet) > 0:
            ...
            self.n = n - 1
            axis = depth % self.n  # axis为切分的坐标轴
            mid = int(m / 2)
            dataSetcopy = sorted(dataSet, key=lambda x: x[axis])
            node = Node(dataSetcopy[mid], depth)  # 创建结点
            if depth == 0:
                self.KdTree = node
    ...
    def search(self, x, count=1):
        ...
        for i in range(count):
            nearest.append([-1, None])  # 第一项表示最近的距离
        ...
        def recurve(node):
            if node is not None:
                ...
                if self.nearest[-n - 1, 0] > abs(daxis):  # 如果区域与圆相交，重新计算最近邻
                    if daxis < 0:
                        recurve(node.rchild)
                    else:
                        recurve(node.lchild)
...
for x in test:
    ...
    show_train()
    plt.scatter(x[0], x[1], c='red', marker='x')  # ‘x’表示测试点
    ...
    for n in near:
        ...
        plt.scatter(n[1].data[0], n[1].data[1], c='green', marker='+')
    plt.legend()
    plt.show()
